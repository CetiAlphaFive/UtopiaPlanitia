...
As of this run, `loco()` no longer depends on the non-CRAN
`conformalInference` package for any code path — its one remaining
non-trivial split-mode case (per-variable, ranger, regression, `loss =
"abs"`) now falls through to the package's own `loco_custom_split()`
...
> mark optional (`Suggests`-only) dependencies. `LOCO` (`R/loco.R`) is
> highlighted blue — its split-mode code path changed in this run (the
> `conformalInference`-calling branch was deleted; the non-CRAN
> `conformalInference` dependency itself is gone from `Suggests` and from
> every reachable code/doc path).
...
| `R/loco.R` | Diagnostics | LOCO variable importance for `ranger`/`grf` **outcome** forests (regression/probability/classification); auto-detects backend; split-sample and OOB modes | `loco()` (+ internal `loco_custom_split()`, `detect_backend()`, `oob_loss()`, etc.) | **yes** — `conformalInference`-calling branch deleted; per-variable/regression/`loss="abs"` split case now falls through to `loco_custom_split()` |
...
> Blue nodes changed in this run. `loco()`'s split-mode branch that used to
> call the external `conformalInference::loco()` (its own internal
> `sample()`/`ranger::ranger()`/`my.z.test`/`my.wilcox.test` sequence) was
> deleted; every split-mode case (including the migrated one) now calls the
> in-package `loco_custom_split()`, which was already used for every other
> split-mode configuration and is unmodified in its own logic — only its set
> of callers grew (implicitly, since the branch above it was removed rather
> than `loco_custom_split()` itself being touched).

### Function Reference

| Function | Defined In | Called By | Calls | Changed | Purpose |
| --- | --- | --- | --- | --- | --- |
| `loco()` | `R/loco.R` | user / exported | `detect_backend`, `treetype_for_backend`, `pred_names_for`, `extract_grf_hp`, `oob_loss`, `loco_custom_split`, `grf_refit` | **yes** | Public entry point; validates args, recovers hyperparameters, dispatches to split-mode (`loco_custom_split`) or OOB mode |
| `loco_custom_split()` | `R/loco.R` | `loco()` (unconditionally, for every split-mode case as of this run) | `ranger::ranger`, `grf_refit`, `stats::pnorm`/`qnorm`, `stats::wilcox.test` | no (callers changed, body did not) | Sample-splits data, refits full + per-target reduced models, computes one-sided Z or Wilcoxon inference on loss-residual differences |
...
> `loco_custom_split()` (highlighted), which was already the destination for
> every other split-mode configuration.
...
- **S3 dispatch is the public interface.** A fitted `causal_forest` flows
  into `summary()` / `plot()`; LOCO objects (`cf_loco`) and test objects
  (`omni_hetero`) carry their own print/summary/plot methods. `loco()` itself
  returns a plain `data.frame` (no S3 class) — it operates on outcome
  forests, not causal forests, and is intentionally simpler.
