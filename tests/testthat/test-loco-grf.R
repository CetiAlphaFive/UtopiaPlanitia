...
test_that("grf regression_forest + OOB + abs returns expected shape", {
  ...
  expect_setequal(out$variable, paste0("x", 1:4))
  ...
  expect_equal(out$variable[1L], "x1")
})
...
test_that("grf regression_forest + split + abs uses custom split loop and gives one-sided p-values", {
  ...
  pv_x1 <- out$p.value[out$variable == "x1"]
  expect_lt(pv_x1, 0.2)
...
test_that("grf regression_forest + split + wilcox", {
  ...
  expect_true(all(out$p.value >= 0 & out$p.value <= 1, na.rm = TRUE))
})
...
test_that("grf regression_forest + group-LOCO (OOB and split)", {
  ...
  expect_setequal(out_oob$variable, c("g_signal", "g_noise"))
  expect_true("members" %in% names(out_oob))
  expect_equal(out_oob$variable[1L], "g_signal")

  out_split <- loco(b$mod, split = TRUE, groups = grp, method = "z",
                    seed = 1, alpha = 0.2)
  expect_setequal(out_split$variable, c("g_signal", "g_noise"))
  expect_true("members" %in% names(out_split))
  expect_lt(out_split$p.value[out_split$variable == "g_signal"], 0.2)
})
...
test_that("grf regression: same seed gives reproducible importances", {
  ...
  ord <- function(o) o[order(o$variable), ]
...
test_that("grf boosted_regression_forest + split + z", {
  ...
  expect_named(out, c("variable", "importance", "ci.lower", "ci.upper",
                      "p.value", "method", "loss"))
  expect_lt(out$p.value[out$variable == "x1"], 0.2)
})
...
test_that("grf probability_forest + OOB + log loss", {
  ...
  expect_true(all(out$loss == "log"))
  expect_true(all(is.finite(out$importance)))
})
...
test_that("grf probability_forest + group-LOCO brier", {
  ...
  out <- loco(b$mod, split = FALSE,
              ...
              seed = 1)
  expect_setequal(out$variable, c("g_sig", "g_noise"))
  expect_equal(out$variable[1L], "g_sig")
})
