...
| `loco.R` | `loco()` | LOCO variable importance for ranger AND grf outcome forests (regression/probability/boosted); auto-detects backend from model class; returns a classed `loco_vimp` object (`$vimp` + metadata). Split mode has an opt-in `cross.fit = TRUE` (`num.folds = 5` default) K-fold path so every observation contributes once instead of only the held-out ~n/2; inference reuses `cf_perm()`'s Nadeau-Bengio corrected SE but references it against **z** (not `cf_perm()`'s `t_{K-1}`) — a simulation found the t-reference over-conservative, masking the power gain; `method = "wilcox"` and OOB (`split = FALSE`) are unsupported with `cross.fit = TRUE` (error) |
